                            ...
                            int>::type* = 0 )
{
    ...
    auto random_coord_op = KOKKOS_LAMBDA( const int p )
    {
        ...
        for ( int d = 0; d < 3; ++d )
            px[d] = Kokkos::rand<RandomType, double>::draw( gen, box_min[d],
                                                            box_max[d] );
        pool.free_state( gen );
    ...
    typename std::enable_if<is_slice<PositionType>::value, int>::type* = 0 )
{
    ...
    auto random_coord_op = KOKKOS_LAMBDA( const int p )
    {
        ...
        // Only increment if no neighbors within min distance.
        auto c = Kokkos::atomic_fetch_add( &count( 0 ), 1 );
        for ( int d = 0; d < 3; ++d )
            positions( c, d ) = px[d];
    };
...
int createParticles(
    InitRandom tag, PositionType& positions, const std::size_t num_particles,
    const double min_dist, const ArrayType box_max, const ArrayType box_min,
    const uint64_t seed = 342343901,
    typename std::enable_if<is_slice<PositionType>::value, int>::type* = 0 )
{
    using exec_space = typename PositionType::execution_space;
    return createParticles( tag, exec_space{}, positions, num_particles,
                            min_dist, box_min, box_max, seed );
}
