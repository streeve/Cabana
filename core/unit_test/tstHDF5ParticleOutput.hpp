...
void checkVector( SliceType1 write, SliceType2 read )
{
    for ( std::size_t p = 0; p < write.size(); ++p )
    {
        ...
        for ( int d = 0; d < write.extent( 2 ); ++d )
        {
...
void checkMatrix( SliceType1 write, SliceType2 read )
{
    ...
    {
        // TODO: update when testing for Views.
        for ( int d1 = 0; d1 < write.extent( 2 ); ++d1 )
            for ( int d2 = 0; d2 < write.extent( 3 ); ++d2 )
            {
...
//---------------------------------------------------------------------------//
void writeReadTest2d()
{
    std::array<double, 3> low_corner = { -2.8, 1.4 };
    std::array<double, 3> high_corner = { 1.2, -7.9 };

    // Allocate particle properties.
    int num_particle = 100;
    using DataTypes = Cabana::MemberTypes<double[2],   // coords
                                          double[2],   // vec
                                          float[2][2], // matrix
                                          int>;        // id.
    Cabana::AoSoA<DataTypes, TEST_MEMSPACE> aosoa( "particles", num_particle );
...
// RUN TESTS
//---------------------------------------------------------------------------//
TEST( HDF5, WriteRead3d ) { writeReadTest3d(); }

TEST( HDF5, WriteRead2d ) { writeReadTest2d(); }
