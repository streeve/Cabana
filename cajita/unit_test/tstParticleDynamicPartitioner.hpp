...
void random_distribution_automatic_rank( int occupy_num_per_rank )
{
    ...
    auto pwm = createParticleWorkloadMeasurer<partitioner.cell_num_per_tile_dim,
                                              partitioner.num_space_dim>(
        TEST_EXECSPACE{}, particle_view, occupy_num_per_rank, global_low_corner,
        cell_size, MPI_COMM_WORLD );
    partitioner.setLocalWorkload( &pwm );
